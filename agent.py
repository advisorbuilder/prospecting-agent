...
def fetch(url, timeout=15, silent_fail=False):
    try:
        ...
        if not silent_fail:
            # only log if not a common block (403/404)
            err = str(e)
...
def fetch_prnewswire():
    ...
    for query, ptype, texas in queries:
        ...
        raw     = fetch(url, silent_fail=True)
        if raw:
...
def fetch_open_news():
    log("Fetching open news RSS feeds...", "📰")
    feeds = [
        ("https://feeds.reuters.com/reuters/businessNews", "Reuters Business", False),
        ("https://www.globenewswire.com/RssFeed/subjectcode/15-Legal+Affairs", "GlobeNewswire Legal", False),
        ("https://www.globenewswire.com/RssFeed/subjectcode/2-Mergers+%26+Acquisitions", "GlobeNewswire M&A", False),
        ("https://www.globenewswire.com/RssFeed/subjectcode/9-Management+Changes", "GlobeNewswire Exec Changes", False),
        ("https://feed.businesswire.com/rss/home/?rss=G22&rssid=22", "BusinessWire Finance", False),
        ("https://feed.businesswire.com/rss/home/?rss=G7&rssid=7", "BusinessWire Legal", False),
    ]
    all_results = []
    keywords = ["partner", "executive", "acquired", "sold", "million", "retires",
                "departs", "joins", "launches", "fund", "equity", "attorney",
                "private equity", "hedge fund", "merger", "acquisition", "Texas",
                "Austin", "Houston", "Dallas", "law firm", "counsel"]
    tx_keywords = ["Texas", "Austin", "Houston", "Dallas", "San Antonio",
                   "Fort Worth", "Midland", "Permian"]
    for url, label, texas in feeds:
        raw = fetch(url, silent_fail=True)
        if not raw:
            continue
        items = parse_rss(raw, label, "open_news", texas)
        filtered = [i for i in items
                    if any(k.lower() in (i["entity"]+i["detail"]).lower() for k in keywords)]
        for item in filtered:
            if any(t in (item["entity"]+item["detail"]) for t in tx_keywords):
                item["texas"] = True
        all_results += filtered[:8]
        time.sleep(0.3)
    log(f"Open news feeds: {len(all_results)} items", "✓")
    return all_results
        ...
        try:
            ...
            try:
                ...
                if not raw_text.endswith("]"):
                    ...
                    raw_text += "}" * max(open_braces, 0) + "]"
                try:
                    batch_scored = json.loads(raw_text)
                except Exception:
                    log(f"Batch {i//batch_size+1} JSON unrecoverable — skipping", "⚠")
                    batch_scored = []
